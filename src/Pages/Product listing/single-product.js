...
const SingleProduct = () => {
    ...
    useEffect(() => {
        ...
        const fetchImages = async () => {
            try {
                const response = await axios.get(`${process.env.BACKEND_URL}/api/v1/products/${id}/images`);
    ...
    const getProductInfo = async () => {
        try {
            const res = await axios.get(`${process.env.BACKEND_URL}/api/v1/products/single-product/${id}`);
    ...
    const handleCart = async (product) => {
        try {
            ...
            const response = await axios.post(
                `${process.env.BACKEND_URL}/api/v1/auth/add-to-cart`,
